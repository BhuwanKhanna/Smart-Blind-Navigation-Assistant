from __future__ import annotations

import smtplib
from email.message import EmailMessage
from typing import Any

import requests

from backend.app.core.config import Settings


class SafetyNotifier:
    def __init__(self, settings: Settings) -> None:
        self.settings = settings

    def send_emergency(self, payload: dict[str, Any]) -> list[str]:
        channels: list[str] = []
        if self.settings.alert_email_enabled:
            channels.append(self._send_email(payload))
        if self.settings.alert_sms_enabled:
            channels.append(self._send_sms(payload))
        return [item for item in channels if item]

    def share_location(self) -> dict[str, Any]:
        return {
            "lat": self.settings.location_sim_lat,
            "lon": self.settings.location_sim_lon,
            "maps_url": f"https://maps.google.com/?q={self.settings.location_sim_lat},{self.settings.location_sim_lon}",
            "status": "shared-simulated",
        }

    def _send_email(self, payload: dict[str, Any]) -> str:
        subject = "[Navigation Assistant] Emergency help requested"
        body = (
            f"Emergency request triggered.\n"
            f"Reason: {payload['reason']}\n"
            f"Location: {payload['location']['lat']}, {payload['location']['lon']}\n"
            f"Time: {payload['timestamp']}\n"
        )

        if not all(
            [
                self.settings.smtp_username,
                self.settings.smtp_password,
                self.settings.smtp_sender,
                self.settings.emergency_contact_email,
            ]
        ):
            print(f"[SIMULATED EMAIL] To={self.settings.emergency_contact_email}\n{body}")
            return "simulated-email"

        message = EmailMessage()
        message["Subject"] = subject
        message["From"] = self.settings.smtp_sender
        message["To"] = self.settings.emergency_contact_email
        message.set_content(body)

        try:
            with smtplib.SMTP(self.settings.smtp_host, self.settings.smtp_port, timeout=10) as server:
                server.starttls()
                server.login(self.settings.smtp_username, self.settings.smtp_password)
                server.send_message(message)
            return "email"
        except Exception as exc:
            print(f"[EMAIL ERROR] {exc}")
            return "simulated-email"

    def _send_sms(self, payload: dict[str, Any]) -> str:
        if not all(
            [
                self.settings.twilio_account_sid,
                self.settings.twilio_auth_token,
                self.settings.twilio_from_number,
                self.settings.emergency_contact_phone,
            ]
        ):
            print(
                f"[SIMULATED SMS] To={self.settings.emergency_contact_phone} "
                f"Emergency help requested at {payload['location']['lat']}, {payload['location']['lon']}"
            )
            return "simulated-sms"

        try:
            response = requests.post(
                f"https://api.twilio.com/2010-04-01/Accounts/{self.settings.twilio_account_sid}/Messages.json",
                auth=(self.settings.twilio_account_sid, self.settings.twilio_auth_token),
                data={
                    "From": self.settings.twilio_from_number,
                    "To": self.settings.emergency_contact_phone,
                    "Body": (
                        f"Emergency help requested. Reason: {payload['reason']}. "
                        f"Location: {payload['location']['lat']}, {payload['location']['lon']}."
                    ),
                },
                timeout=10,
            )
            response.raise_for_status()
            return "sms"
        except Exception as exc:
            print(f"[SMS ERROR] {exc}")
            return "simulated-sms"
